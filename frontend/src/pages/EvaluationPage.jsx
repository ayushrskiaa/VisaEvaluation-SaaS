...
export default function EvaluationPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Step 2: Upload document (this triggers AI analysis on backend)
      setUploading(true);
      try {
        await visaApi.uploadDocument(id, 'resume', uploadedFile);
        console.log('Document uploaded successfully. AI analysis complete.');
      } catch (uploadErr) {
        console.error('Upload error:', uploadErr);
        setError(uploadErr.response?.data?.error || 'Failed to upload document');
        setUploading(false);
        return;
      }

      // Show results (evaluation now contains AI analysis)
      setShowResults(true);
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mb-8 text-center">
          <Link to="/" className="inline-block mb-4 text-blue-600 hover:text-blue-700 text-sm font-medium">
            ← Back to Home
          </Link>
